...
from resume_builder.api.schemas.jobs import JobListResponse
...
async def queue_job(
    ...
    Accepts a URL, raw text, or an uploaded .txt file via multipart form.
    """
    if file:
        content = await file.read()
        task_id = await service.queue_job_file(
            user_id, session_id, content, file.filename or "job.txt"
        )
    elif url or text:
        task_id = await service.queue_job(user_id, session_id, url, text)
    else:
