...
# ------------------------------------------------------------
# Interactive / conversational session models
# ------------------------------------------------------------


class ConversationEntry(BaseModel):
    """A single turn in the interactive conversation."""
    ...
    intent: str = Field(description="Classified intent label, e.g. 'add_project'")
    result_summary: str = Field(
        default="", description="Brief outcome of the operation"
    )
...
class InteractiveResumeState(BaseModel):
    """
    State for the InteractiveResumeFlow conversational loop.
    Persisted after each step to survive restarts.
    """

    session_id: str = Field(default="", description="Unique session identifier")
    resume_path: str | None = Field(
        default=None, description="Path to original resume PDF"
    )
    working_resume: ParsedResume | None = Field(
        default=None, description="The evolving resume — incrementally updated"
    )
    parsed_job_postings: list[JobRequirements] = Field(
        default_factory=list,
        description="Job postings accumulated for tailoring",
    )
    tailored_resumes: list[TailoredResume] = Field(
        default_factory=list,
        description="Tailored resumes generated by tailor over accumulated jobs",
    )
    conversation_log: list[ConversationEntry] = Field(
        default_factory=list, description="Full interaction history"
    )
    errors: list[str] = Field(default_factory=list)
