...
class TestFlowInit:
    def test_requires_resume_source(self) -> None:
        """Flow must have non-empty resume_raw_text."""
        with pytest.raises(ValueError, match="resume_raw_text cannot be empty"):
            ResumeBuilderFlow(resume_raw_text="", job_postings_raw=["job"])
    ...
    def test_requires_job_postings(self) -> None:
        ...
        with pytest.raises(ValueError, match="Provide at least one job posting"):
            ResumeBuilderFlow(resume_raw_text="text", job_postings_raw=[])

    def test_accepts_valid_inputs(self) -> None:
        flow = ResumeBuilderFlow(
            resume_raw_text="John Smith\nEngineer\nSkills: Python",
            job_postings_raw=["Job 1 text"],
        )
    ...
    def test_accepts_multiple_jobs(self) -> None:
        flow = ResumeBuilderFlow(
            ...
            job_postings_raw=["Job A", "Job B", "Job C"],
        )
    ...
    def test_copies_job_postings(self) -> None:
        ...
        jobs = ["Job A"]
        flow = ResumeBuilderFlow(resume_raw_text="text", job_postings_raw=jobs)
...
class TestFlowProgress:
    def test_on_progress_callback(self) -> None:
        ...
        flow = ResumeBuilderFlow(
            resume_raw_text="Test resume",
            job_postings_raw=["Test job"],
...
class TestFlowStepValidation:
    ...
    def test_crew_requires_parsed_resume(self) -> None:
        ...
        flow = ResumeBuilderFlow(resume_raw_text="text", job_postings_raw=["job"])
